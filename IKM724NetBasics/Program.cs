...
namespace FireSuppressionSystem
{
    class Program
    {
        ...
        private static FireControlSystem fireControlSystem = new FireControlSystem();
        ...
        static async Task Main(string[] args)
        {
            ...
            {
                {"RoomTemperature",() => new TemperatureSensor("Temperature", "Room temperature sensor") },
                {"SmokeDetector",() => new SmokeSensor("Smoke", "Smoke level detector") },
            ...
            foreach(var indicator in deserializedResult)
            {
                if(sensorFactories.TryGetValue(indicator.Name, out var createSensor))
                {
                    ...
                    fireControlSystem.Sensors.Add(sensor);
                }
                else 
                {
        ...
        {
            fireControlSystem.Monitor();

            var value = fireControlSystem.Sensors.FirstOrDefault(x => x.Name == indicatorModel.Name);
